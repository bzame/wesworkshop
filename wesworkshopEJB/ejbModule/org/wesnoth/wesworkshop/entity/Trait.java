...
@NamedQueries({
    ...
    , @NamedQuery(name="Trait.findById", query="SELECT t FROM Trait t WHERE t.id = :id")
    , @NamedQuery(name="Trait.findByTraitGameId", query="SELECT t FROM Trait t WHERE t.gameId = :gameId")
})
public class Trait extends WesnothObject implements Serializable {
    ...
    private TraitAvailability availability;
    /**
    ...
    */
     ...
     * or "none" if it is not normally available, but should be kept when advancing to this unit type. 
     */
    public TraitAvailability getAvailability() {
        return availability;
    }
     ...
     */
    public void setAvailability(TraitAvailability availability) {
